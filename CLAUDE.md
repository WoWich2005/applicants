...
- **Консультант по разделу 7 «Экономическое обоснование»:** Т. Н. (параметры расчёта — в памяти `diploma_economics_consultant.md`: норма дисконта = ставка реф. + 2 п.п., ФСЗН = 34%).
- **Раздел 8 «Охрана труда»** написан по учебнику А. М. Лазаренкова и санитарным нормам РБ (СанПиН, типовые инструкции по охране труда при работе с ПЭВМ).
...
├── images/                 # рисунки (.png, .pdf) — ~26 шт.
├── styles/                 # .bst-файлы (не используются)
├── reference/              # методичка, нормоконтроль, чужие дипломы как образцы (не в сборке)
├── word/                   # внешние листы, оформленные в Word и сохранённые как PDF
│   ├── 1 Лист задания стр.1.{doc,pdf}   # лист задания (печатается отдельно, НЕ в main.tex)
│   ├── 2 Лист задания cтр.2.{doc,pdf}   # лист задания, стр. 2 (НЕ в main.tex)
│   ├── 4 Ведомость.{doc,pdf}            # ведомость объёма ДП — \includepdf в main.tex
│   └── 6 Графическая часть.{doc,pdf}    # графическая часть (печатается отдельно, НЕ в main.tex)
├── build/                  # вывод компиляции (main.pdf, main.aux, main.toc, …)
└── sections/
    ├── toc.tex             # оглавление
    ├── referat.tex         # реферат (стр. 3, \input ДО оглавления)
    ...
    ├── chapter7.tex        # 7 Экономическое обоснование (консультант Т. Н.)
...
**Порядок в `main.tex`:** референт → `\includepdf` ведомости → оглавление → введение → главы 1–8 → заключение → список источников → приложения.
Лист задания (стр. 1–2) и графическая часть оформлены в Word (`word/`), в `main.tex` **не подключаются** — печатаются и подшиваются отдельно. В сборку через `\includepdf` входит только `word/4 Ведомость.pdf`.

**Статус (2026-06-01):** записка **полностью завершена** — все 8 разделов, введение, заключение, список источников и приложения написаны; собирается в `build/main.pdf` без ошибок.
...
**Всегда** через макрос `\citesrc{key}`, никогда через жёсткие числа `[5]` и больше **не** через ручной `[\ref{src:key}]`. Макрос определён в `preamble.tex`:

```latex
\newcommand{\citesrc}[1]{\unskip~\mbox{[\ref{src:#1}]}}
```

Он сам ставит неразрывный пробел перед ссылкой и оборачивает её в `\mbox`, чтобы `[N]` не разрывался по строкам:

```latex
...применяется более чем в 1500 учреждениях\citesrc{banner}.
```
...
Допустимо: **по порядку появления в тексте** или **по алфавиту**. Сейчас источники упорядочены **по мере первого упоминания в тексте** и сгруппированы по разделам комментариями (`% --- Раздел N ... ---`) — это финальный порядок.

#### При добавлении источника

1. Дописать `\src[key] ...` в `bibliography.tex` (в нужную по порядку упоминания группу)
2. В тексте ссылаться как `\citesrc{key}`
...
- `\citesrc{key}` — ссылка на источник в тексте (см. выше); единственный способ цитировать
- `\rv{...}` — кириллическая переменная в math-режиме (оборачивает в `\text{}`, т.к. math-шрифт XeLaTeX не содержит кириллицы). Объявляется через `\providecommand` локально в `chapter7.tex`/`chapter8.tex`
- `\No` — устойчивый знак №
- **Счётчики реферата** (в `preamble.tex`): `\totfig` / `\tottab` / `\totref` — итог рисунков/таблиц/источников, `\pageref{lastmainpage}` — номер последней страницы заключения. Подставляются в `referat.tex` автоматически после двух прогонов компиляции.
