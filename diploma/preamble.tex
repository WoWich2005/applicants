...
% --- Графика ---
% Расширения файлов перечислены в порядке предпочтения: при ссылке
% \includegraphics{name} без расширения LaTeX возьмёт .pdf (вектор)
% если он есть, иначе .png. Это позволяет хранить и PDF (для печати),
% и PNG (для предпросмотра).
\usepackage{graphicx}
\usepackage{rotating}
\usepackage{float}
\graphicspath{{images/}}
...
% Условная вставка иллюстрации: если файл существует — вставляется,
% иначе рисуется текстовая заглушка-прямоугольник. Это позволяет
% собирать документ при отсутствии реальных скриншотов UI и т.п.
% Использование:
%   \figureorstub[0.7\textwidth]{login}{Скриншот страницы входа}
\newcommand{\figureorstub}[3][\textwidth]{%
  \IfFileExists{images/#2.pdf}{%
    ...
    \IfFileExists{images/#2.png}{%
      ...
      \fbox{\parbox[c][50mm][c]{\dimexpr#1-2\fboxsep-2\fboxrule\relax}{\centering\itshape [Заглушка: #3]}}%
    }%
  }%
}
...
% В таблицах:
%   - \codearg переопределяется на seqsplit-вариант, чтобы длинные URL
%     и идентификаторы могли разрываться (вне таблиц \code не разбивается);
%     сам \code как точка входа остаётся прежним — он по-прежнему отключает
%     babel-shorthand для " и передаёт аргумент в \codearg;
...
\AtBeginEnvironment{longtblr}{%
  ...
  % Заменяем растяжимый клей seqsplit между буквами на нерастяжимый
  % discretionary — точка разрыва есть, но межбуквенный пробел не «расползается»
  % при выключке по ширине в X-ячейках.
  \def\seqinsert{\discretionary{}{}{}}%
  \let\codearg\codeargtable%
}
\usepackage{multirow}
\usepackage{makecell}
...
% Параметры размещения плавающих объектов: разрешаем рисункам занимать
% больше места на странице, чтобы они «всплывали» к ближайшему удобному
% положению (top/bottom/here), а не уезжали на отдельную страницу,
% оставляя пустоту в основном тексте.
\renewcommand{\topfraction}{0.9}        % до 90% верха страницы — float
\renewcommand{\bottomfraction}{0.7}     % до 70% низа — float
\renewcommand{\textfraction}{0.1}       % минимум 10% страницы — текст
\renewcommand{\floatpagefraction}{0.7}  % страница float'ов считается заполненной от 70%
% placeins: \FloatBarrier гарантирует, что плавающие объекты не «уплывают»
% за границу раздела. Опция [section] вставляет барьер автоматически
% перед каждым \section, чего достаточно для нашей структуры (разделы — \section).
...
% TOC-запись без точечного заполнителя и без номера страницы.
% Используется для ведомости объёма, графической части и приложений —
% по образцу оформления (методичка п. 3.8 / 4.3.2): эти элементы выводятся
% одной строкой без указания страниц.
\newcommand{\tocnopagenum}[1]{%
  \addtocontents{toc}{%
    \protect\par
    \protect\noindent #1%
    \protect\par
...
% --- Вставка внешних PDF-страниц (для графической части) ---
% pdfpages позволяет включать готовые PDF-файлы как страницы документа.
% Используется в приложении для вставки графической части (слайдов
% презентации, оформленных в Word и сохранённых как PDF).
\usepackage{pdfpages}

% --- Листинги программного кода ---
\usepackage{listings}
\usepackage{xcolor}
% Локально внутри lstlisting понижаем глобальные penalties (см. начало преамбулы:
% \hyphenpenalty=10000, \exhyphenpenalty=10000, \tolerance=9999). Без этого
% listings не может разорвать длинные строки кода, потому что breaklines
% использует штрафы переноса, которые мы запретили глобально для основного
% текста на русском.
\AtBeginEnvironment{lstlisting}{%
  \hyphenpenalty=50\exhyphenpenalty=50\tolerance=200%
}
\lstset{
...
% В тексте ссылаться как \citesrc{banner} — номер обновляется автоматически.
\newcommand{\src}[1][]{\stepcounter{totreferences}\item\ifx\relax#1\relax\else\label{src:#1}\fi}

% \citesrc{key} — ссылка на источник в тексте; не переносится на новую строку.
% Поглощает пробел перед собой и заменяет его на неразрывный (\unskip~),
% а сам номер с квадратными скобками заворачивает в \mbox.
...
% \code{...} — семантический маркер для идентификаторов/путей/строк кода.
% Визуально неотличим от обычного текста (нормоконтроль БНТУ запрещает другой
% шрифт для кода вне приложений). В longtblr переопределяется на seqsplit
% (см. \AtBeginEnvironment{longtblr} выше) для переноса длинных идентификаторов.
%
% Двухступенчатая реализация нужна, чтобы внутри аргумента не срабатывали
% babel-russian shorthand'ы (особенно активный символ "). Без \shorthandoff{"}
% последовательности вида "Key", id съедают закрывающую кавычку и запятую.
\newcommand{\codeargnormal}[1]{#1\endgroup}
\newcommand{\codeargtable}[1]{\seqsplit{#1}\endgroup}
\let\codearg\codeargnormal
% Внутри \code делаем " активным и раскрывающимся в \textquotedbl, чтобы
% получить настоящий прямой ASCII-quote (U+0022). Без этого babel-russian
% преобразует " в типографский ” (U+201D), а соседние символы рискуют
% попасть под shorthand-разбор (например, "Key", id → ”Key id).
% \begingroup ... \endgroup гарантирует scoped catcode только внутри \code.
\begingroup
\catcode`\"=\active
\gdef\code{\begingroup\catcode`\"=\active\let"\textquotedbl\codearg}
\endgroup

% --- Оформление приложений с исходным кодом (по образцу нормоконтроля БНТУ) ---
% \appendixhead{буква}{подзаголовок} — заголовок приложения по центру:
%   первая строка ПРИЛОЖЕНИЕ X (жирный), вторая строка — описание (обычный).
% \codefile{имя_файла} — нумерованный подзаголовок «N) Код файла <имя>»
%   с абзацным отступом 1.25 см. Счётчик сбрасывается при каждом \appendixhead.
\newcounter{codefile}
\newcommand{\appendixhead}[2]{%
  \clearpage
  \setcounter{codefile}{0}%
  \begin{center}
  \textbf{ПРИЛОЖЕНИЕ~#1}\\[6pt]
  #2
  \end{center}
  \tocnopagenum{ПРИЛОЖЕНИЕ~#1}%
  \vspace{12pt}
}
\newcommand{\codefile}[1]{%
  \par\vspace{6pt}%
  \noindent\hspace{1.25cm}\stepcounter{codefile}\thecodefile)\hspace{0.5em}Код файла \code{#1}%
  \par\vspace{6pt}%
}
